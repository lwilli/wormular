...
11. **Visit + play counters** — cookieless first-party `POST /visit`, `POST /play?mode=…`, `GET /stats` on the same Worker/D1 (aggregate visits + solo/local/online plays). **Done:** ADR [adr/0003-cookieless-visit-analytics.md](adr/0003-cookieless-visit-analytics.md).
