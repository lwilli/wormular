...
- Title screen is the **paused starting arena** (same rocks, worm, apple as the run) with a wrapping circle-carousel mode picker (Solo / Local 1v1 / Online 1v1 always visible as center + side peeks of real mini arenas), mode title under the logo, mode-specific chrome, and a Press & Hold prompt. Swipe or tap peeks to select; first hold/tap starts the selected mode (title presses are deferred briefly so a swipe does not also launch). Side peeks stay inset enough that their short labels remain readable on narrow phones. Solo shows personal high score + leaderboard (no mode tagline); Local shows the dual-control brief; Online shows nickname, with ready / queue / error copy in the same center prompt. Death returns to a fresh paused arena with the last mode still selected.
