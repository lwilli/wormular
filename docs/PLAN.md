...
- Title screen is the **paused starting arena** (same rocks, worm, apple as the run) with a wrapping circle-carousel mode picker (Solo / Local 1v1 / Online 1v1 as a scaled selected arena plus separate side peek disks), mode title under the logo, mode-specific chrome, and a Press & Hold prompt. Swipe or tap peeks to select; first hold/tap starts the selected mode (title presses are deferred briefly so a swipe does not also launch; start expands the arena while chrome fades). Side peeks stay inset enough that their short labels remain readable on narrow phones. Solo shows personal high score + leaderboard (no mode tagline); Local shows the split-touch control brief (orange top / teal bottom); Online shows nickname, with ready / queue / error copy in the same center prompt. Death opens a **result screen** (final score / winner) that stays until tap, then returns to a fresh paused arena with the last mode still selected.
...
**State machine:** `Title` | `Playing` | `Result`. After death (and death juice), show a result overlay with the run score / winner; tap dismisses back to `Title` with a fresh paused play world. The first hold unpauses that world (no spawn swap).
...
**Title:** full-bleed **paused** play arena so the player can find the worm and rocks before time starts. Overlay: logo (top), mode title/tagline centered between logo and arena core (Solo has no tagline), **start prompt** (center; not a click target — Solo: Press & Hold; Local: Tap to Start → 5s countdown; Online: Tap to find an opponent, then Finding / Waiting / error in that same prompt), wrapping side-circle peeks for the other two modes (separate mini canvas arenas beside a slightly scaled-down selected mode, with short labels), and a bottom panel that swaps by mode (Solo: nickname + personal high score + leaderboard; Local: orange top-half / teal bottom-half control brief; Online: nickname only). Swipe horizontally or tap peeks to change mode (wraps) — selection does not start a run; title presses defer start until the gesture is clearly a tap/hold rather than a swipe. Solo shows the solo start pose; Local/Online 1v1 show both worms opposite at equal radius (local reuses that layout on start). After death, a **result overlay** shows the score / winner on the dimmed final arena until tap, then a new paused world appears behind the title overlay with the mode selection preserved.

**Playing:** no HUD except a small current score (Local 1v1: dual mirrored seat chrome — orange top rotated 180°, teal bottom upright — plus soft zone tints). Solo: finger/click anywhere is thrust. Local: top half thrusts orange, bottom half thrusts teal (multi-touch); Space / W still work on keyboard. On death: play juice, then hold a result screen (Solo: score / new high; Local/Online: winner + both scores) until tap returns to the title overlay.

**Result:** centered overlay (`#result`, mirrored into local seats for Local 1v1) with headline, score detail, and “Tap to continue”. Input is gated like the title restart cooldown so a held death mash cannot dismiss instantly.
...
10. **1v1 battle** — dual-worm sim (opposite starts, 2 apples, first death loses); local split-input (top/bottom touch halves + Space/W); online lockstep via Durable Objects. **Done:** local + online; `MatchRoom` DO; finish-before-close; view mirror; match countdown; input pipelining; mobile split-touch zones.
